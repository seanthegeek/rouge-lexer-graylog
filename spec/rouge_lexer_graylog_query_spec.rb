...
class RougeGraylogQueryLexerTest < Minitest::Test
  def setup
    @lexer = Rouge::Lexers::GraylogQuery.new
  end

  def test_finds_by_tag
    assert_equal Rouge::Lexers::GraylogQuery, Rouge::Lexer.find('graylog-query')
  end

  def test_finds_by_alias_graylog_search
    assert_equal Rouge::Lexers::GraylogQuery, Rouge::Lexer.find('graylog-search')
  end

  def test_guesses_by_mimetype
    assert_equal Rouge::Lexers::GraylogQuery, Rouge::Lexer.guess(mimetype: 'text/x-graylog-query')
  end
  ...
  def load_demo   = File.read(File.join(__dir__, 'demos', 'graylog_query'))
  def load_sample = File.read(File.join(__dir__, 'visual', 'samples', 'graylog_query'))
